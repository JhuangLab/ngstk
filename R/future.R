...
par_download <- function(urls, destfiles = NULL, logdir = sprintf("%s/logs", tempdir()), 
                         ids_func = function(urls) {stri_rand_strings(n = length(urls), length = 20)},
                         logfn_func = function(ids, logdir){sprintf("%s/%s.log", logdir, ids)},
                         parallel_method = "plan(multiprocess)", ...) {
  is_fine <- is.character(urls)
  if (!all(is_fine)) warning(sprintf("%s is not character.", paste0(urls[!is_fine], collapse = ", ")))
  if (is.null(destfiles)) destfiles <- basename(urls)
  ...
  file.create(logs)
  msg <- sprintf("Downloading %s (log id: %s) destfile is %s. Log saved to %s.", urls, ids, destfiles, logs)
  sapply(msg, message)
  flush(stderr())
  status <- list()
