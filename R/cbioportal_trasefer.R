...
muts2mutation_mapper <- function(input_data, input_type = "iseq", config_file = system.file("extdata", "config/cbioportal.toml", 
  ...
  this_section <- "muts2mutation_mapper"
  meta_flag <- "meta"
  format_flag <- "format"
  ...
  for (i in 1:length(defined_cols)) {
    hander_data <- cbioportal_handler(hander_data, config_input, defined_cols, input_data, i, hander_funs, 
      hander_extra_params)
  }
  hander_data <- cbioportal_mhandler(hander_data, config_input, mhander_funs, mhander_extra_params)
  if (!is.null(outfn)) {
...
# main interface for cbioportal data process
cbioportal_handler <- function(hander_data, config_input, defined_cols, input_data, index, hander_funs = NULL, 
  extra_params = NULL) {
...
cbioportal_mhandler <- function(hander_data, config_input, mhander_funs = NULL, extra_params = NULL) {
